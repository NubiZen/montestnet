...
class Application {
  ...
  async initialize() {
    try {
      ...
      this.dashboard.updateCycles(0, config.cycles.default);
      const services = {
  ...
  async runCycle() {
    try {
      ...
      this.dashboard.updateCycles(this.cycleCount, config.cycles.default);
...
if (require.main === module) {
  ...
  process.on('SIGTSTP', () => {
    console.log('[Main] Ctrl+Z detected. Exiting gracefully...');
    process.exit(0);
  });
  ...
  async function processWallets() {
    ...
    while (true) {
      try {
        ...
        dashboard.updateLog("[Main] Waiting 24 hours before the next iteration...");
        dashboard.render();
        await dashboard.showCountdown(24 * 60 * 60 * 1000);
      } catch (error) {
        ...
        const errorDashboard = new Dashboard();
        errorDashboard.updateLog("[Main] Waiting 24 hours before the next iteration despite the error...");
        errorDashboard.render();
        await errorDashboard.showCountdown(24 * 60 * 60 * 1000);
      }
    }
  }
